...
public:
     ...
     {
          header_.insert( { move( name ), move( value ) } );
          return *this;
     }
     ...
     {
          output << "HTTP/1.1 " << static_cast< int >( resp.httpCode_ ) << ' ' << resp.GetCodeComment() << '\n';
          for( auto const& [ key, value ]: resp.header_ )
          {
               output << key << ": " << value << '\n';
          }
...
private:
     HttpCode httpCode_;
     multimap< string, string > header_;
     string content_;
};
