...
struct Hasher {
     std::size_t operator() ( const Point3D& p ) const
     {
          ...
          return (hCordType( p.x ) * x * x + hCordType( p.y ) * x + hCordType( p.z ));
     }

     std::hash<CoordType> hCordType;
};
...
void TestDistribution() {
     ...
     // ???? ???‚?°?‚?????‚???‡?µ???????? ?·???°?‡?????????‚???? 0.95:
     //
     // ?µ???»?? ?????????‚?°?????‚?? ?????µ???‚?? ++buckets[hasher(point) % num_buckets]
     // ???‹?ˆ?°?¶?µ?????µ ++buckets[dist(gen) % num_buckets], ?‚?? ?? ???µ?ˆ?????‚???????‚???? 95%
     // ASSERT ?????¶?µ ???‚?ˆ?°?±???‚?°?µ?‚ ???????µ??????,
     //
