...
public:
     ...
     struct WriteAccess
     {
          ...
          WriteAccess( mutex& m, MapType& map, const K& key )
          : lock( m )
     ...
     struct ReadAccess
     {
          ...
          ReadAccess( mutex& m, const MapType& map, const K& key )
          : lock( m )
     ...
     {
          SafeMap& sm = maps_[ hasher( key ) % maps_.size() ];
          return WriteAccess( sm.m, sm.map, key );
     }

     ReadAccess At( const K& key ) const
     {
          const SafeMap& sm = maps_[ hasher( key ) % maps_.size() ];
          return ReadAccess( sm.m, sm.map, key );
     }

     bool Has( const K& key ) const
     {
          const SafeMap& sm = maps_[ hasher( key ) % maps_.size() ];
          lock_guard< mutex > lock( sm.m );
          return sm.map.count( key ) > 0;
     }

     MapType BuildOrdinaryMap() const
     {
          MapType result;
          for( const SafeMap& sm: maps_ )
          {
               std::lock_guard< std::mutex > lock( sm.m );
               for( const auto& [ key, value ]: sm.map )
               {
                    result[ key ] = value;
               }
          }
...
void TestConstAccess() {
  ...
  vector<future<string>> futures( 10 );
  for (int i = 0; i < 10; ++i) {
